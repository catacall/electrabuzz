...
import { Building2, Users, Vote, CalendarDays, ChevronDown, ChevronUp, HelpCircle, CheckCircle, XCircle } from "lucide-react";
...
export default function LokSabhaPage() {
  ...
  const [quizState, setQuizState] = useState<{ current: number; score: number; answered: boolean; selected: number | null }>({
    current: 0, score: 0, answered: false, selected: null,
  });

  const lokSabhaConstituencies = constituenciesData.filter(c => c.type === "Lok Sabha");
    ...
    <div className="flex flex-col gap-10 p-4 sm:p-8 max-w-5xl mx-auto pb-20">
      ...
      <div className="text-center">
        <div className="inline-flex items-center gap-3 t-card px-6 py-3 rounded-full border t-border mb-4">
          ...
          <span className="text-sm font-bold t-accent uppercase tracking-wider">Lower House of Parliament</span>
        </div>
        <h1 className="text-4xl sm:text-5xl font-extrabold t-text tracking-tight">Lok Sabha</h1>
        <p className="t-muted mt-3 max-w-xl mx-auto">The House of the People â€” the heart of Indian democracy where 543 elected members represent over 1.4 billion citizens.</p>
      </div>
      ...
      <div className="space-y-3">
        <h2 className="text-2xl font-bold t-text mb-4">Learn About Lok Sabha</h2>
      ...
      {/* Representatives */}
      <div>
        <h2 className="text-2xl font-bold t-text mb-4">Notable Constituencies</h2>
            ...
            <div key={c.id} className="t-card border t-border rounded-2xl p-5 hover:t-card transition-all hover:-translate-y-1 shadow-sm group">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-bold t-text2">{(c.voters / 100000).toFixed(1)}L</p>
                </div>
              </div>
            </div>
          ))}
